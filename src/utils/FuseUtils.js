...
import { flatten, join, values } from 'ramda';
...
class FuseUtils {
    ...
    static getErrorMessageFromResponse(err) {
        let msg = 'Internal Server Error.';
        if (err && err.response && err.response.data) {
            if (err.response.data.errors) {
                msg = join(' ')(flatten(values(err.response.data.errors)));
    ...
    static currencyFormatWithSign(amount) {
        if (amount === '') {
            return '';
        }
        return new Intl.NumberFormat({ style: 'currency', currency: 'USD' }).format(amount);
    }
}
